...
The identity hash includes upstream, authorization, organization, project, session, and model. It is not derived from `prompt_cache_key` because callers may share that key across conversations. Authentication rotation opens another lane and can reduce reuse; it cannot mix two credential scopes.
