...
Choose the allowed task-quality regression and minimum cost improvement before looking at results. Require no loss of active tool items, no state commits on failed or interrupted generations, and no cross-session projection reuse. The [inline receipt](opaque-replay.md) verifies recursive provider compaction through Astral with two random tool fixtures, no plaintext canaries in final input, and replay after proxy restart. The subsequent [three-arm benefit trial](benefit-evaluation.md) validates Astral-owned inline checkpoints across two rollovers and restart on both task seeds. Astral's standalone backend still needs a live endpoint implementing its contract. An empty projection surviving restart does not satisfy that gate.

The benefit trial passed its correctness and state gates but found slower complete tasks than full history and more input tokens than provider compaction. It reported usage rather than verified spend, with limited cache warming. Those results do not satisfy a general cost or performance release gate.
