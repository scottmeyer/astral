...
fn implementation(root: &Path, subsystem: &str, apply: Option<&str>) -> Result<Value> {
    ...
    let id = subsystem.strip_prefix("subsystem:").unwrap_or(subsystem);
    let observation = project.freshness.get(id).ok_or_else(|| {
        error(
            "FRESHNESS_NOT_CONFIGURED",
            "choose a subsystem with an explicit freshness.inputs declaration",
        )
    })?;
    ...
    let plan_sha256 = crate::fingerprint(&json!({
        "schema_version": 1, "root": root, "subsystem": id,
    ...
    let mut result = json!({
        ...
        "scope": "Acknowledges review of the declared code files against current core and subsystem/dependency documents. Does not run tests, save a session, stage files or verify prose correctness."
    });
